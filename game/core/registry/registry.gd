...
## Replaces any previous scan on success; a failed scan leaves the
## previous index untouched. Returns OK or the first error found.
func scan(dir_path: String) -> Error:
	var defs := {}
	var err := _scan_dir(dir_path, defs)
	if err != OK:
		return err
	_defs = defs
	return OK
...
func _scan_dir(dir_path: String, defs: Dictionary) -> Error:
...
func _index(def: Definition, path: String, defs: Dictionary) -> Error:
	if def.id == &"":
